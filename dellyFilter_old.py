...
for fname in glob.glob('*.vcf'):
    ...
    for r in fin:
        ...
        prat = (1.0 * pDV / pe)
        if (pDV >= 25 and rDV <= 1) or (8 <= pDV < 25 and rDV == 0 and 0.8 <= prat <= 1.0) or (3 < pDV < 8 and rDV == 0 and pri['RV'] >= 1 and 0.8 <= prat <= 1.0 and ref['RV'] == 0):
    ...
    print "  {:<15} {:>6} [{:.2f}%]".format("Small Fail", small_fail, 100.0 * small_fail / total)
    #print "  {:<15} {:>6} [{:.2f}%]".format("Reference Fail", ref_fail, 100.0 * ref_fail / total)
    #print "  {:<15} {:>6} [{:.2f}%]".format("Primary Fail", pri_fail, 100.0 * pri_fail / total)
