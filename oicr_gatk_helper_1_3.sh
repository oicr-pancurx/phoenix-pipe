...
#index input bam
	...
	echo "qsub -cwd -N ${SGE_PREFIX}${bname}_index -e $LOG_DIR -o $LOG_DIR -b y -l h_vmem=6G \"module load java/$JAVAVER; module load picard/$PICVER; time java -Xmx4g -Djava.io.tmpdir=\${TMP_DIR} -jar \\\${PICARDROOT}/BuildBamIndex.jar INPUT=${i} VALIDATION_STRINGENCY=LENIENT TMP_DIR=./tmp\""
done
...
#realigner target creator
	...
	for j in ${chr[@]} ; do
		...
		echo "qsub -cwd -N  ${SGE_PREFIX}${bname}_realignerTargetCreator  -hold_jid  ${SGE_PREFIX}${bname}_index -e $LOG_DIR -o $LOG_DIR -l mf=20G -l h_vmem=20G -b y \"bash ./$SCRIPT_DIR/RealignerTargetCreator_${j}.sh \" ";		
	done
	
#indel realigner
	echo "# Indel realigner"
	for j in ${chr[@]} ; do
		echo -n  "module load java/$JAVAVER; module load gatk/${VER}; time java -Xmx40g -Djava.io.tmpdir=./tmp/ -jar \${GATKROOT}/GenomeAnalysisTK.jar $BAMINPUT -R $FASTA -T IndelRealigner -L ${j} -targetIntervals ./${INTERVAL_DIR}/${bname}.${j}.intervals -o ./${BAM_DIR}/${bname}.realigned.${j}.bam -known $DBSNP --maxReadsInMemory 10000000 -et NO_ET" > ./$SCRIPT_DIR/IndelRealigner_${j}.sh
	
		echo "qsub -cwd  -N  ${SGE_PREFIX}${bname}_indelRealigner -hold_jid  ${SGE_PREFIX}${bname}_realignerTargetCreator -e $LOG_DIR -o $LOG_DIR -l h_vmem=48G -b y \"bash ./$SCRIPT_DIR/IndelRealigner_${j}.sh \" ";
		done
...
#count covariates for recalibration
	...
	echo "qsub -cwd -b y -N ${SGE_PREFIX}${bname}_countCovariates -hold_jid ${SGE_PREFIX}${bname}_fixmate -e $LOG_DIR -o $LOG_DIR -l h_vmem=20G  \"bash ./$SCRIPT_DIR/CountCovariates.sh \" ";
...
#table recalibration 
	...
	for j in ${chr[@]} ; do 
		echo "qsub -cwd  -N  ${SGE_PREFIX}${bname}_table_recal -hold_jid ${SGE_PREFIX}${bname}_countCovariates -e $LOG_DIR -o $LOG_DIR -b y -l h_vmem=20G \"bash ./$SCRIPT_DIR/TableRecal_${j}.sh \" ";
	done
...
#merge indel output
 ...
 # TAB This requires updating, and implementation of variant quality score recalibration
	...
	for j in ${chr[@]} ; do
		echo "qsub -cwd  -N ${SGE_PREFIX}${bname}_indelFiltering -hold_jid ${SGE_PREFIX}${bname}_unifiedGenotyperIndel -e $LOG_DIR -o $LOG_DIR -l h_vmem=20G -b y \"bash ./$SCRIPT_DIR/indelFilter_${j}.sh \""
	done
...
# TAB This requires updating, and implementation of variant quality score recalibration
	...
	for j in ${chr[@]} ; do
		echo "qsub -cwd  -N ${SGE_PREFIX}${bname}_snpFiltering -hold_jid ${SGE_PREFIX}${bname}_unifiedGenotyper,${SGE_PREFIX}${bname}_unifiedGenotyperIndel -e $LOG_DIR -o $LOG_DIR -l h_vmem=20G -b y \"bash ./$SCRIPT_DIR/snpFilter_${j}.sh \" ";
	done
...
echo "qsub -cwd -b y -N ${SGE_PREFIX}${bname}_clean_tmp_files -hold_jid ${SGE_PREFIX}${bname}_snpFiltering -e gatkLog -o gatkLog \"rm bam_files/*.ba?\""
